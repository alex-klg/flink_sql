...
-- Define table
-- 在 trino 执行 show ca
CREATE TABLE kf_optimism_transactions
(
   hash STRING,
   nonce bigint,
   ...
   receipt_effective_gas_price bigint
    ...
    'connector' = 'kafka',
    'topic' = 'chain_data_optimism_transactions',
...
SET 'pipeline.name' = 'optimism_transactions_sync';
-- SET 'table.dml-sync' = 'true';

-- select CAST('' AS bigint)
-- select to_timestamp(1666781543) as ts
-- select to_timestamp(from_unixtime(1666781543, 'yyyy-MM-dd HH:mm:ss'))
-- select * from kf_optimism_transactions;
insert into iceberg.beta_bronze.optimism_transactions
select
